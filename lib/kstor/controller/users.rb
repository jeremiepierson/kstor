...
module KStor
  ...
  module UserController
    def group_create(name)
      ...
      group_id = @store.group_create(name, keypair.pubk)
      encrypted_privk = Crypto.encrypt_group_privk(
        @user.pubk, keypair.privk
      )
      Log.debug("encrypted_privk = #{encrypted_privk}")
      keychain_item_create(group_id, keypair.pubk, encrypted_privk)

      Model::Group.new(id: group_id, name: name, pubk: keypair.pubk)
    end
